...
import { requestDeleteAccountOTP, confirmDeleteAccount, logout } from '../../../services/api/auth/deliveryAuthService';
import { motion, AnimatePresence } from 'framer-motion';

export default function DeliveryDeleteAccount() {
  ...
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await requestDeleteAccountOTP();
      if (response.success) {
        setOtpSent(true);
  ...
  const handleConfirmDeletion = async (e: React.FormEvent) => {
    ...
    if (!window.confirm('CRITICAL WARNING: This action is permanent. Your delivery partner profile, earnings history, and wallet will be deleted forever. This cannot be undone. Do you really want to proceed?')) {
      return;
    }

    setLoading(true);
    try {
      const response = await confirmDeleteAccount(otp, sessionId);
      if (response.success) {
        setSuccess(true);
        setTimeout(() => {
          logout();
      ...
      setError(err.message || 'Deletion failed. Please check the OTP or try again later.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md mx-auto">
        ...
        >
          <div className="bg-red-600 p-8 text-white text-center relative overflow-hidden">
            ...
            <h1 className="text-2xl font-bold uppercase tracking-tight">Delete Delivery Account</h1>
            <p className="text-red-100 text-sm mt-1">Permanently remove your delivery partner profile</p>
          </div>
            ...
            <AnimatePresence mode="wait">
              {success ? (
                ...
                >
                  ...
                  <p className="text-neutral-500">Your delivery account has been closed successfully. You will be redirected shortly.</p>
                </motion.div>
              ) : (
                <motion.div key="form">
                    <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl text-amber-800 text-xs leading-relaxed">
                        <b>Warning:</b> Deleting your account will immediately remove your profile from MandiBazaar. All your earnings history and wallet balance will be lost permanently.
                    </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              disabled
                              className="w-full bg-neutral-100 rounded-2xl border-none pl-16 pr-6 py-4 text-sm font-medium text-neutral-500"
                              required
                            />
                          </div>
                        </div>
                      ...
                      <>
                        ...
                        <div className="bg-red-50 p-4 rounded-2xl border border-red-100 flex items-start gap-4 mb-4">
                            ...
                            <label htmlFor="confirmCheck" className="text-xs text-red-700 font-medium leading-tight">
                                I understand that this will permanently delete my delivery profile and all balance.
                            </label>
                        </div>
