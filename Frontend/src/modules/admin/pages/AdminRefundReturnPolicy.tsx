...
export default function AdminRefundReturnPolicy() {
  ...
  const fetchPolicy = async () => {
    try {
      ...
      console.error('Error fetching refund/return policy:', error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        showToast('Refund & Return Policy updated successfully!', 'success');
      } else {
        showToast(response.message || 'Failed to update policy', 'error');
      }
    } catch (error) {
      console.error('Error updating refund/return policy:', error);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white px-4 sm:px-6 py-4 border-b border-neutral-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-neutral-900">Refund & Return Policy</h1>
          </div>
          <div className="text-sm text-neutral-600">
            <span className="text-blue-600">Home</span> / <span className="text-neutral-900">Refund & Return Policy</span>
          </div>
        </div>
      </div>
              ...
              <div className="p-4 sm:p-6">
                <div>
                  <label className="block text-sm font-bold text-neutral-800 mb-2">
                    Refund & Return Policy Text <span className="text-red-500">*</span>
                  </label>
                  <textarea
                    name="returnPolicy"
                    value={returnPolicy}
                  ...
                  <p className="mt-2 text-xs text-neutral-500">
                    This content will be displayed to customers in the "Refund & Returns" section of the Legal & Policies page.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="p-4 sm:p-6">
                <div className="prose max-w-none">
                  <div className="whitespace-pre-wrap text-sm text-neutral-700 bg-neutral-50 p-4 rounded border border-neutral-200 min-h-[200px] max-h-[400px] overflow-y-auto font-sans">
                    {returnPolicy || 'Policy content will appear here...'}
                  </div>
                </div>
              </div>
            </div>
