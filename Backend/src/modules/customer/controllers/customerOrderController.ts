    ...
    try {
        ...
        for (const item of items) {
            ...
            if (!product) {
                ...
                if (product && product.stock <= 5) {
                    const { sendStockAlertNotification } = await import("../../../services/notificationService");
                    try {
                        ...
                        console.error("Error sending stock alert notification:", notifyError);
                    }
                }
            }
        ...
        try {
            ...
            if (io) {
                ...
                const savedOrder: any = await Order.findById(newOrder._id).populate('items').lean();
                if (savedOrder) {
...
export const cancelOrder = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Notify
        try {
            const { sendSellerOrderCancelledNotification, sendCustomerOrderNotification } = await import("../../../services/notificationService");
            ...
            for (const sellerId of sellerIds) {
                try {
                    await sendSellerOrderCancelledNotification(sellerId, order.orderNumber, order.customerName || 'Customer');
                } catch (notifyError) {
                    console.error("Error sending cancellation notification to seller:", notifyError);
                }
            }
            ...
            try {
                ...
                console.error("Error sending cancellation notification to customer:", notifyError);
            }
